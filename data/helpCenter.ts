...
// verbatim from pc.jin57.cc — short functional Q&A copy, consistent with
// how the rest of this demo handles UI/marketing text (confirmed identical
// wording to WU88's real site, just with "WU88" swapped for "JIN"). The
// 關於我們 legal pages (規則與條款 / 隱私權政策) are long-form legal text on
// the real site (also identical to WU88's, brand name swapped), so they're
// paraphrased/condensed here rather than reproduced at length, per this
// project's standing copyright policy.
...
export const FAQ_ITEMS: FaqItem[] = [
  {
    q: "允許我在JIN遊戲嗎？",
    a: "您必須已經年滿18歲或所在當地多數的法定年齡，以最大年齡為準，並且已經閱讀並同意JIN規則與條款。",
  },
  {
  ...
  {
    q: "在JIN儲值與託售，我的註冊姓名是否需要與身分證上面的姓名一致？",
    a: "您在JIN註冊帳戶，必須使用真實合法的姓名和身份，且您在JIN的投注帳戶用於接收資金的銀行帳戶必須保持一致。",
  },
  {
    q: "我每天可以提交多少筆儲值交易？",
  ...
  {
    q: "如何更改和取消已確認的注單？",
    a: "閣下有責任確保投注的正確性，一旦您的投注經由我們確認並接受，注單將不能取消，撤消及更改；您的投注被認為確認無誤。所有JIN輸入的投注記錄將存入JIN交易資料庫，並作為任何時間任何交易的有效依據。",
  },
  {
  ...
  {
    q: "為什麼會出現重新派彩的情況？",
    a: "JIN最初公佈的比分或結果需要修正時，將會出現賽果變更。出於結算的目的，賽事的結果將會在當天確定，除非在個別的體育或賽事規則有說明。賽果均在賽事結束後判定，除非在個別運動規則裡另有註明。賽果公佈72小時後，若對任何賽果有爭議，本公司將不認可。在賽果公佈72小時內,除了任何體育紀律委員會所重新裁決之賽果，本公司只會修正人為、系統或參考賽果的相關網頁失誤等原因的錯誤。",
  },
  {
    q: "JIN哪個管道取得賽事結果？",
...
export const RULES_AND_TERMS: AboutDoc = {
  title: "規則與條款",
  paragraphs: [
    "本頁說明會員存取與使用JIN網站服務所應遵循的權利義務。請在接受本站服務前詳細閱讀本協議；一旦完成註冊或使用本站服務，即代表您已閱讀並同意本協議（以下稱「網站」或「本站」）的所有規則與條款，以及站內不時公告或更新的優惠規則與其他相關政策。",
...
export type TutorialFlow = {
  flow: string;
  count: number;
  /** "dots" = numbered page buttons; "counter" = "X / Y" with prev/next
   * arrows. pc.jin57.cc uses the counter style for every tutorial tab
   * (confirmed live), unlike WU88 which mixes both styles. */
  pagination: "dots" | "counter";
};

// 超商搜尋流程 only has one store option here (7-11查詢) — no 全家查詢
// toggle, unlike WU88.
export const STORE_SEARCH_FLOW: TutorialFlow = { flow: "storesearch-711", count: 5, pagination: "counter" };
// USDT虛擬貨幣 splits into two sub-flows via its own inner tab bar.
export const USDT_DEPOSIT_FLOW: TutorialFlow = { flow: "usdt-deposit", count: 5, pagination: "counter" };
// BitoPro 流程 itself further splits into 3 sub-flows, selected via a
// dropdown menu (arrow icon next to the "BitoPro 流程" label) - confirmed
// live on pc.jin57.cc: 下載註冊認證 is the default/active option.
export type BitoproSubFlow = "下載註冊認證" | "購買 USDT" | "查看交易紀錄";
export const BITOPRO_SUB_FLOWS: { key: BitoproSubFlow; flow: TutorialFlow }[] = [
  { key: "下載註冊認證", flow: { flow: "usdt-bitopro-register", count: 14, pagination: "counter" } },
  { key: "購買 USDT", flow: { flow: "usdt-bitopro-buy", count: 6, pagination: "counter" } },
  { key: "查看交易紀錄", flow: { flow: "usdt-bitopro-history", count: 3, pagination: "counter" } },
];
